...
import { ArrowUpRight, ChevronDown, RefreshCw, ShieldCheck, Terminal, UsersRound } from '@lucide/vue'
import { getOAuthList, refreshOAuthAccount } from '../services/oauth.js'
...
async function refreshCredential(account) {
  if (refreshingIds.value.has(account.id) || disposed) return
      ...
      <div v-else-if="loaded" class="account-grid">
        <article v-for="account in accounts" :key="account.id" class="account-card" :aria-label="account.email || account.accountId || '授权账号'">
          ...
          <footer class="card-footer">
            <button class="credential-refresh" :disabled="refreshingIds.has(account.id)" :aria-busy="refreshingIds.has(account.id)" :aria-label="`刷新 ${account.email || account.accountId || '账号'} 的授权凭证`" title="刷新授权凭证" @click="refreshCredential(account)">
