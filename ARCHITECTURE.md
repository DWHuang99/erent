...
flowchart LR
    ...
    API -->|OIDC discovery / authorization-code exchange| OIDC[OIDC Provider]
...
- `cmd/api/main.go` 只编排进程生命周期：先初始化 JSON logger 并预检日志文件，再由 `config.go` 加载运行配置和可选 OAI OIDC 配置，由 `instances.go` 连接已迁移的 PostgreSQL/Redis 并创建 Repository、Casbin、JWT、OIDC 等实例；`bootstrap.go` 负责初始用户，`health.go` 负责健康检查，`routes.go` 负责 Gin、中间件和业务路由注册，最后由 `main.go` 调用 `router.Run(HTTP_ADDR)`；
...
- OAuth/OIDC：通用 Handler/Service 负责一次性 state、PKCE、回调和 token exchange；provider 包负责 discovery 后的 OAuth 配置与授权 URL 参数；
...
Auth 与 User 的分层不产生进程间网络调用，均位于同一 `cmd/api` 进程内；启用 OIDC 时，API 启动阶段会向 issuer 执行 discovery，回调阶段会请求 discovery 得到的 token endpoint。
...
- 已实现：可选 OIDC discovery、一次性 Redis state、Authorization Code + PKCE、通用 OAuth Handler/Service 和 OAI 路由骨架；
